...
export default function MonthlyBreakdownTable({ data = [], loading = false }) {
  ...
  if (loading) {
    return (
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-2xs h-[300px] flex items-center justify-center">
        <p className="text-xs text-slate-400 font-medium">Loading monthly records...</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100">
            {sortedRecords.map((item, index) => {
              ...
              return (
                <tr key={item.REVENUE_MONTH || index} className="hover:bg-slate-50/60 transition-colors">
                  <td className="py-2.5 pl-1 font-medium text-slate-900">
                  ...
                  <td className="py-2.5 pr-1 text-right font-mono font-semibold text-slate-900">
                    {formatCurrency(rev)}
                  </td>
                </tr>
              );
